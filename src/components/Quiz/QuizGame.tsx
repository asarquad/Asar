...
export default function QuizGame() {
  ...
  const syncPoint = async (change: number, isCorrect: boolean, isMilestone: boolean) => {
    ...
    try {
      ...
      let currentXp = 0;
      if (userSnap.exists()) {
        currentXp = typeof userSnap.data().xp === 'number' ? userSnap.data().xp : 0;
      }
      
      // Prevent negative XP
      const actualChange = (currentXp + change < 0) ? -currentXp : change;

      const updates: any = {
        xp: increment(actualChange),
  ...
  const handleAnswer = async (optionIndex: number) => {
    ...
    if (isMilestoneQuestion) {
      ...
      setUserTotalXp(prev => Math.max(0, prev + pointChange));
